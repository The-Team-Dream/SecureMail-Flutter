...
class Alertsscreen extends ConsumerWidget {
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Error loading notifications', style: AppTextStyles.headingS),
                            const SizedBox(height: 8),
                            Text(state.error!, style: AppTextStyles.bodyM, textAlign: TextAlign.center),
                    ...
                    : alerts.isEmpty
                        ? Center(
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Icon(Icons.notifications_off_outlined, size: 64, color: context.text3.withOpacity(0.5)),
                                const SizedBox(height: 16),
                                Text('No notifications yet', style: AppTextStyles.headingS.copyWith(color: context.text3)),
                                const SizedBox(height: 8),
                                Text('You will see security alerts and system updates here.', style: AppTextStyles.bodyM.copyWith(color: context.text3), textAlign: TextAlign.center),
                              ],
                            ),
                          )
                        : RefreshIndicator(
  ...
  Widget _buildAlertCard(BuildContext context, AlertNotification alert) {
    ...
    return InkWell(
      onTap: () {
        if (alert.emailId != null && alert.mailBoxId != null) {
          context.push(
            AppRoutes.messageDetail(alert.mailBoxId!),
            extra: MailboxMessage(
              id: alert.emailId!.toString(),
              mailboxId: alert.mailBoxId,
              initials: '??',
              sender: '...',
              subject: alert.title,
              preview: alert.description,
              timeLabel: '...',
              badgeLabel: alert.category.name.toUpperCase(),
